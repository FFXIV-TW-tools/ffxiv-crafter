...
> 未做（需 Owner 拍板）：讓職業專屬 icon **跟著配方職業走**（做金工配方卻顯示木工版鋸子）。需在 monorepo `build_game_ref.py` 的 `craft_actions` 加 `class_job_id` 欄並重建 `game_ref.sqlite`＝跨 repo 資料模型變更，已記 `docs/BACKLOG.md` B-008。
