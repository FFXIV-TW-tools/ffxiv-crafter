...
## 2026-08-26 — 行動適配：配方列表由「9 欄橫捲表」改「逐列堆疊卡」

portal 側把行動適配從「逐工具 opt-in」改成預設要做（`_DESIGN-SYSTEM.md` §📱 行動適配是預設，
不是選配）。本站實測是破口第二嚴重的一個。

**破口**（393px）：`#recipe-table` scrollWidth **960** / clientWidth **357**（`.rt` 帶
`min-width: 960px`）⇒ 可視區只剩名稱欄，種類／職業／Lv／配方等級／難度／品質／版本／加入鈕
**全在畫面外**，而捲軸是隱藏的 ⇒ 使用者看到的是一張「只有名字」的清單，且沒有任何訊號說右邊還有東西。

**修法**（≤700px）：`min-width` 歸零、表格 `display:block`、thead 隱藏、每列改
**flex-wrap 卡片**（名稱整寬，其餘欄自然併行換行），加入鈕絕對定位釘右上。
- ⚠️ **第一版用 grid 是錯的**：欄寬差距太大（「錬金原料」vs「Lv 100」），grid 逼每格各佔一列 →
  長成 6 行的鬆散卡。flex-wrap 才會讓短欄併行。
- ⚠️ **右側預留 92px 不是估的**：`.rt-actwrap` 是**固定兩格**的 inline-grid（＋ 恆在第一欄、
  − 恆在第二欄，2026-08-19 Owner 裁示），實寬 80–84px。第一版照「單顆鈕 40px」寫 52px，
  結果「難度 4125」被壓在鈕底下——量過才發現那格是 80px。
- **DOM 只加不減**：四個純數字欄補 `data-label`（桌面不顯示，只有手機的 `::before` 讀它）。

**驗證**：393px `#recipe-table` scrollWidth == clientWidth（357，橫捲消失）、列高 80px、
逐列量測「內容右緣 vs 加入鈕左緣」無重疊；桌面 1366 停動畫逐像素 A/B 零版面回歸
（差異只有貓提示氣泡與 navbar 角標）、`.rt` 在桌面仍 `table-row`／`min-width:960px` 不變。
`test-formulas` 653→**654**（新增互鎖，見 `docs/test-baseline-history.md`）、
`tests/run-all` 4/4、`check-actions` 35 個變體全綠。

## 2026-08-19（三）· 追加 3 — 版本篩選 ／ 最低能力要求管控

### 版本篩選（Owner 需求）

- 成品的實裝版本來自 `item_lookup.items.patch`（**13874 筆全有值，零缺漏**），由 `build-data.py`
  寫進 `items.json` → RINDEX。配方表加「版本」欄（9 欄），篩選器 `#patch-filter` 的選項**由資料生成**。
- 分法＝Owner 裁示：**繁中服開服即 7.0** ⇒ 7.0 以前併成一個選項（10651 筆），7.0 之後按實際
  有配方的版號分（13 個，各帶筆數）。刻意不寫死版號清單——資料一更新就靜默漏配方。
- ⚠ 版號比較一律 `parseFloat`：拆成 (major, minor) 整數比會把 **7.15 排到 7.5 後面**（minor 15 > 5），
  而下拉看起來仍然「有排序」⇒ 只有逐項對答案才看得出來。哨兵在 T11。
- 資料片譯名刻意不用：台服解包沒有 ExVersion 表，照繁中服至上鐵則不自創譯名，一律顯示版號。

### 最低能力要求：原本完全沒管控（Owner 提問查出）

13874 個配方裡 **3396 個**有 `required_craftsmanship`／`required_control`，而全站對這兩欄
**零引用** ⇒ 數值不夠的人照樣求解、照樣複製巨集，進遊戲才發現製作筆記不給做（站上零訊號）。

- 單一出口 `statShortfall(recipe, gear)`：配方詳情顯示「需求 作業 X 加工 Y」（不足轉紅＋⚠＋
  hover 寫出還差多少），`doSolve` 擋下並導去角色數值分頁。
- ⚠ 比較基準是 `effectiveStats`（含食物／藥水／專家之證）——遊戲判定同樣吃 buff，拿裸裝比會誤擋。
- 求解鈕走 `aria-disabled` 不用真 disabled（同「缺角色數值」的既有取捨：真 disabled 鍵盤走不到、
  螢幕閱讀器讀不到原因）。實測「破布木木材」（需求 5380／4650）：需求列轉紅、按下去被擋、
  未開始求解、跳到角色數值分頁。

測試 627 → **653 passed**（T11 擴充 ＋ 新增 T60／T61）。

## 2026-08-19（三）· 追加 2 — 內容井抽成共用 `.crafter-well`

Owner：「製造清單跟素材總需求的內 table 應該要是暗色的吧，做出對比；配方求解頁已經內外分明了。」

- 配方求解頁那層「內容井」（比 panel 深一階的實底 `--color-bg` ＋ accent 32% 染框 ＋ 6px 圓角）
  原本只寫在 `.recipe-table` 上 ⇒ 抽成 `.crafter-well`，由**配方表／配方清單／素材總需求**三處共用。
  不是各自再塗一次暗色：值一樣時畫面看不出差別，但幾何就分岔成三份事實源。
- 分層實測：卡片 `rgb(16,24,36)` ／ 井 `rgb(8,12,19)`；配方表改用共用 class 後底色・邊框・圓角
  與改前逐項相同。素材三組各一口井，組標題留在卡片上（標題屬外、資料屬內）。

測試 621 → **627 passed**（T59）。

## 2026-08-19（三）· 追加 — 配方表高度吃滿當前螢幕

Owner：「依次顯示一個螢幕能顯示的配方數量就好，然後表內滾動，不要我還要滾動外表格，大概 15 筆左右。」

- 表格高度由寫死的 `max-height: 60vh` 改成 `CraftBrowse.fitHeight()` 實量：
  可用高度＝視窗高 −表格上緣 −（`<main>` 底緣 − 表格底緣）−8，`resize` 時重算（debounce 120ms）。
  **不用魔術常數**：上方的高度會隨窄屏折行而變、翻頁器有時整條收起。
  1215px 視窗實測＝571px／**15 列**，內容底離視窗底 6px；篩到剩 1 筆時表格縮成 109px（不留空井）。
- ⚠ 實作中踩到：拿 `document.scrollHeight` 反推「下方佔多高」會越縮越小（489→419→349）——
  portal 的 body 公式 `min-height:100vh` ＋ `padding-top:64px` 使**文件高度恆為 100vh+64、與內容無關**。
  已改用 `<main>` 底緣，並加原始碼哨兵禁止 `scrollHeight` 回到該函式。
- 極矮視窗（實測 ≤800px 高、且職業 chips 折兩行時上方就吃掉 514px）收在下限約 6 列：
  寧可外層捲一點，也不把表格壓成一條縫。

測試 617 → **621 passed**。

## 2026-08-19（三）— 配方表資訊密度 ／ 高難度標示與篩選 ／ 素材總需求分組

Owner 逐項回報：「明明內容不多為何還要這麼多行才有滾動軸」「名稱跟類別擠在一起」「加入按鈕有好看一點的嗎」
「素材總需求太陽春」「高難度配方要標出來也要能篩」「取消不該非得到清單頁才做得到」「＋− 位置要固定」。

### 配方表：兩行變一行、空間換資訊

- 種類由「名稱下方的副行」拉成**獨立欄**，並補上**難度／品質**兩欄（8 欄）。兩個數字走既有的
  `recipeMaxes(recipe, rlv)`（RINDEX 建索引時算一次）—— **沒有第二份公式**；缺 rlv 列顯「—」不顯 0。
- 列高 55px → **35px**（同視窗可見列數 13 → 22）。⚠ `<td>` 的 `height` 是內容盒下限、padding 與 border
  另外加，舊值宣告 46 其實畫出 55px ⇒ 這類數字一律以量測為準（實測 `height:auto` 亦為 35）。
- 8 欄在窄屏用表格 `min-width: 880px` ＋ 容器橫捲（📊 表格佈局穩定鐵則場景 1）。
  實測 1400/1018/900/800/430/390/360 七個寬度：文件皆無水平溢出、品名不被壓成 0 寬。
- 加入鈕由全形「＋」改**內嵌向量**（畫法對齊 portal `FFXIVIcons` 的 viewBox／stroke-width／linecap）。
  沒有加進共用圖示組——那組刻意只收「已在 2 個以上 repo 被 emoji 頂替過」的圖示，plus 目前只有本站用
  （rule of two 未達）；第二個 repo 需要時再提 Owner 升格。

### 高難度（expert）＝配方屬性，不是名字

- 列表掛 `高難度` 徽章 ＋ 新增 `#expert-filter` 三態（全部／只看／排除）。實測「只看」＝ **536 筆**，
  與 `recipes.json` 的 `is_expert` 計數逐筆相符。
- 實作當下踩到一次「控件加了但沒接 `change`」——畫面有下拉、按了沒反應、console 全乾淨、
  直呼 `renderTable` 的測試也照樣綠 ⇒ 補了接線的原始碼斷言（T11）。

### 就地增減：＋ 旁邊多一顆 −

- 每列 −＝退一次（減到 0 整筆移除），不必切到製造清單。− 恆 render、用 `hidden` 收合
  （`markListState` 是 in-place 更新，改成條件 render 會重建 DOM 吃掉焦點）。
- **槽位固定**＝定寬兩欄 grid：flex 下 − 被收掉時整組會重新置中、＋ 往左跳一格，
  而剛按完「加入」的游標正好停在 − 上 ⇒ 下一下就誤點成移除。
- 兩顆都不上 `--danger`：同質可重加物件的列級增減走設計系統的列級豁免（一表 60 顆紅 ✕＝紅色的狼來了）。

### 素材總需求：依「接下來要做什麼」分三組

- ⚒ 可自製中間材（給「加進清單」入口，**傳的是做幾次不是要幾個**）／🛒 採集購買（商人有賣的掛價格徽章）
  ／💠 晶體。挑配方走 `CraftRecipe.pickRecipeForItem`、商人徽章走 `CraftQuests.vendorHtml`
  ——**都是既有實作，本層不另刻一份**。卡頭補「N 種 · 合計 M 個」。
- 新增 `CraftList.addRuns(id, n)`（一次加 N 次、只噴一次 toast、撞單筆上限誠實說只加到幾次）
  與 `CraftList.removeOne(id)`。

測試 577 → **617 passed**（T11 擴充 ＋ 新增 T58）；逐條說明見 `docs/test-baseline-history.md`。

## 2026-08-17（日）— 「繼續做」下一階反查 ／ 動作列瘦身

### 「用這個成品還能做什麼」（Owner 需求）

既有的「先做這個」只走得下去（成品 → 中間材）。做完一批棕櫚糖後想知道它還能拿去做什麼，
站上唯一的路是自己回配方表逐個搜——**而反查資料早就在 `ingredients.json` 裡**（配方 → 素材），
倒過來建索引即可，**沒有新增任何資料檔**。

- 新層 `app-nextcraft.js`（`globalThis.CraftNext`，173 行）：反查索引 ＋ 下一階選取視窗。
- **為什麼是彈出視窗**（Owner 拍板）：實測 975 件成品有下一階，消費者配方數中位 13、**最多 234**
  （綠金錠）⇒ 就地展開會把求解區推到天邊。視窗走 portal 共用 `.codex-modal`（a11y 契約：ESC／
  點遮罩關閉／`FFXIVA11y.trapFocus`／鎖背景捲動），本地只寫版面特化。
- **一件成品只佔一列**：同一件東西常好幾個職業做得出來（棕櫚糖的 31 個配方裡「特製酵母」佔兩個），
  挑法沿用 `pickRecipeForItem`（玩家有填數值的職業優先），進去之後詳情頁本來就有職業切換鈕。
  棕櫚糖＝31 配方 → **30 列**。
- 篩選：搜尋品名／職業（選單只列這批資料真有的職業，不給死選項）／「只顯示我能做的」（預設開）。
  **fail-safe**：勾著卻一筆都不剩時自動放開並說明——空清單讀起來像「這東西不能再往下做」，正好相反。
- **往上走不推返回堆疊**（Owner 拍板）：一路往上做會堆出一長串用不到的返回點；選到的正好是堆疊
  最上層（剛才鑽下來的來源）時等同按「← 回」並彈掉，否則畫面會出現「← 回 A」而人就站在 A 上。

### 真人才踩得到的 bug：視窗開了又立刻被自己關掉

點「繼續做」→ 視窗閃一下就沒了，**console 全乾淨、程式化 `.click()` 完全測不出來**。
根因：滑鼠在按鈕上按下、放開時遮罩已經蓋在游標底下 ⇒ 那一發 click 的 target 變成遮罩，
被「點遮罩關閉」吃掉。修法＝遮罩關閉必須 **mousedown 也落在遮罩上**（順帶修好「在視窗內按住
拖曳選字、放開時滑到遮罩」的誤關）。T56 有哨兵；新開 modal 一律照這條寫。

### 動作列瘦身（Owner 回報：字太多、想降低整體高度）

- 「📋 加入製造清單」→「📋 加入清單」、「💰 材料樹與利潤」→「💰 查價」（說明仍在 `data-help`）。
- 「⚒ 繼續做（N）」不放配方詳情動作列，改住頂部「目前配方」那一列——**那裡本來就有整片空白，
  等於不佔任何新高度**，詳情動作列也少一顆、窄屏更不容易擠成兩行。沒有下一階時整顆收起。
- 實測 `.ri-head` 67px → **54px**；窄屏 430／390／360px 三種寬度零水平溢出、品名最小 120px。
...
### 提示音不該害玩家多存一格巨集（Owner 回報，同日第二輪）

上一輪把單段容量壓成 14 步後，**剛好 15 步的手法被切成 14+1 兩段**——為了一行提示音，玩家要多存一格
巨集、多按一次，代價遠大於少一聲。修法兩條：

- **剩下剛好 15 步的末段整段塞滿、不補 echo**（15 步 → 1 段、29 步 → 14+15 兩段）。
  只有末段可能命中（14 的切法讓中段恆為 14）⇒ 中段的「第 N 段完成」不會被吃掉。
- **提示音改成可關**（`#macro-echo`，預設開，偏好存 `ffxiv-crafter-macro-echo-v1`）。關掉時單段回到 15 步。
  它不是求解輸入 ⇒ 切換**只重組巨集、不作廢求解結果**（不進 `invalidateResults()`）。

T39 擴充：段數 golden 表（開／關 × 1/13/14/15/16/28/29/40/45 步）＋「總行數 − echo 行數 == 步數」
（echo 逐段可選後，原本的「總行數 − 段數」對帳已不成立）＋開關接線（偏好讀得回來、change 有掛、
切換當場重組）。基線 478 → 528。
