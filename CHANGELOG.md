...
- `data/vendors.json`：職業任務相關的 591 件物品中，**256 件解包確認 NPC 有賣**、其中 38 件另有地點與單價。
  **兩個來源責任分清楚**——「有沒有得買」＝解包 `item_lookup.is_gil_shop`（權威、全覆蓋）；
  「在哪買、多少錢」＝社群試算表（部分覆蓋，UI 標明社群整理）。試算表說有賣但解包說沒有的 **1 件已剔除**：
  寧可少講一句，也不要叫玩家跑一趟空的。
- 地名還原：試算表為排版把地名縮成兩字（「北黑-私語北」），用它自己首頁的縮寫對照表展開成
  「黑衣森林北部林區-私語北」——縮寫只有原作者看得懂。
...
- T33：需 HQ 的商人徽章分流（要 HQ→「只賣 NQ」且不得沿用可購買樣式／未知也提醒／`hq` 欄型別／木工 Lv25 需 HQ・Lv10 不需 HQ 正反 golden）。262 → **297 passed**。
