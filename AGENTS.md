...
| `app.js` | 前端控制器（module 入口）：資料載入 / 公式 computeSettings / 選配方 selectRecipe / 配方詳情 refreshSelectedGear / 消耗品公式套用 / 分頁 / init 接線（**592 行**（wc -l，pre-commit gate 同法），B-002＋B-007＋引導改造拆分後；角色數值/渲染/求解編排/配方瀏覽表/流程引導/食藥選單已抽出，同名 proxy 委派 CraftGear/CraftBrowse） |
...
<!-- TEST-BASELINE cmd="node tools/test-formulas.mjs" match="(\d+) passed, \d+ failed" expect="239" label="test-formulas" -->
