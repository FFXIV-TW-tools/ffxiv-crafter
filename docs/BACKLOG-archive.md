# BACKLOG 封存 — ffxiv-crafter

> **本檔為歷史歸檔，非現行約束。** 已結案（完成／否決）的 B-NNN 條目，自 [`BACKLOG.md`](BACKLOG.md) 搬來，**原文一字未改**。
> 搬移原因＝DEVLOOP §4.5 第二觸發（主檔 >40KB），2026-08-15 建檔。純搬移，revert 即復原。
> 待辦與待拍板一律看主檔；這裡只供回溯「當初為什麼這樣決定」。
