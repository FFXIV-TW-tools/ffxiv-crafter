...
- [ ] **B-039** (P3, test)【建議 中｜延遲風險 低｜執行風險 中（3270 行測試檔要按主題拆、harness 共用要抽層）｜副作用 canonicalTest 的 test-formulas 命令改成跑目錄；TEST-BASELINE 標記跟著改｜體量 M】**`tools/test-formulas.mjs` 3270 行過 2000 紅線，B-037 ② 補上 `.mjs` 後 gate 3 每次接觸都會印紅線警告**——這正是那條修法要的效果，但警告只在「下次實質接觸必須拆分」被履行時才停。候選：按既有 T 編號的主題分成 `tools/tests/` 下的數支（公式 golden／配方表與篩選／配方詳情與製作鏈／求解與巨集／職業任務／靜態哨兵），共用的 vm sandbox 工廠（`LAYER_STUBS`／`makeEl`／`mkT25Ctx`）抽成 `tools/tests/_harness.mjs`；入口改成掃描目錄跑全部並彙總 passed 數（match 正則不變）。⚠️ 拆分不得改任何斷言；每檔 ≤500 行。 來源: 2026-09-06 B-037 落地後 gate 3 首次對本檔發出紅線
