...
- [x] **B-032** (P3, correctness)【建議 中｜延遲風險 低｜執行風險 低｜副作用 選 A＝少一個匯出與 8 條斷言；選 B＝多一條 golden｜體量 S】**`craftPlan` 生產端零呼叫點（死碼），且鑽石依賴會重複計數**（app-recipe.js:62-89（craftPlan / walk），匯出於 :365；T51 的 8 條 golden 全是樹狀）。UI 走的是 `craftIngredient`／返回堆疊與 `crafting-list` 的單層 `aggregateMats`，`craftPlan` 在生產路徑上完全到不了；但 AGENTS.md 架構表把它列為「製作鏈」的實作、T51 有 8 條全綠斷言 ⇒ **一份看起來被驗證過的假護欄**。T51 的三層案例是鏈狀不是鑽石狀，剛好避開缺陷：成品同時直接與間接用到同一個可製作中間材時（FFXIV 常態，例如成品要「亞麻布」也要一個由亞麻布做的部件），底層素材會重複計數——實測 W 應為 4 卻算成 6。同層的 `app-quests.js:54-72 expandMats` 面對同一情境是對的（往下傳 need 不傳 total）。**需拍板**：A（建議）刪掉 `craftPlan` 與 T51 對應斷言、AGENTS.md 架構表同步改口，對齊 2026-08-15 刪三支死 proxy 的既有處置；B 保留並修正遞迴（先完整彙總 need、再單獨一輪展開）＋補鑽石依賴 golden；C 維持現狀（＝這條 finding 本身）。⚠️ 選 A 要順手加一條掃描斷言：`app-recipe.js` 匯出的每個純函式名至少在 index.html 載入的某支 .js 內被呼叫。 來源: 健檢 2026-09-05（R5） ✓ 完成於 cycle 2026-09-05-R5（Owner 拍板 A：刪 craftPlan 與 T51；T64 匯出必有生產端呼叫點掃描；commit 8e9e1df）

- [x] **B-033** (P2, docs)【建議 中｜延遲風險 中｜執行風險 低｜副作用 常駐 context 變小；敘事要搬得動｜體量 M】**`AGENTS.md` 已長回 36,911 B —— R7 豁免的立論已被證偽，且沒有任何閘會發現**（AGENTS.md 開頭的 `R7-exempt: 2026-11-16` 段）。豁免文字寫「已照 R7 規定做完前兩步…**36.1KB → 28.3KB**。剩下的每一條都是可執行規則，再砍就是 R7 自己警告的『為壓 byte 刪有效紅線』」。實測：豁免落地那個 commit（`0ca22de`，2026-08-17）是 **31,248 B**，現在 **36,911 B**（+18%），**已超過當初觸發瘦身的 36.1KB**。成長集中在三段：資料與求解 +2,743／UI 設計系統 +1,587／VERIFY +1,333。⚠️ **形狀比「沒有哨兵」更難處理**：`check-devloop-artifacts` 本輪實跑當場印 `⚠ R7 常駐層 AGENTS.md 36.0KB > 20KB` 後仍判「✓ 工件格式合格」，而該哨兵**不消費 `R7-exempt` 標記**（全檔 grep 無 `exempt`）⇒ 對「已豁免」與「超出豁免當時的值」印的是同一行 ⇒ 這行從豁免那天起就一直在每次 commit 印、一直被合理地當成已知而略過，檔案就長了 18%。**修法必須讓兩者在機械上可分辨**，否則加再多警告都會被同樣略過。豁免到期日還有兩個月。**需拍板**：A（建議）把三段新增內容裡「已固化成測試的條目」壓成「規則一行＋測試編號」、敘事移進 `docs/lessons.md`，目標回 ~30KB；B 提前撤銷豁免、照 R7 正常流程重做瘦身；C 只加哨兵不動內容。⚠️ **C 建議無論選 A 或 B 都一起做**（加一條「AGENTS.md 位元組數不得超過豁免當時的值」的機械斷言）——否則下一次超標還是靠人記得，而這正是本條之所以存在的原因。 來源: 健檢 2026-09-05（R5） ✓ 完成於 cycle 2026-09-05-R5（Owner 拍板 A＋C：38,974 → 30,895 B，9 段敘事搬 lessons；T65 位元組哨兵）

- [x] **B-034** (P3, refactor)【建議 中｜延遲風險 低｜執行風險 中（動 styles.css 要逐頁目視對照）｜副作用 新增數個檔案；index.html 的載入清單與 T49／T53 涵蓋率閘要同步｜體量 M】**三個檔跨過 500 行 review 閘門而無拆分紀錄（app.js 547／styles.css 787／build-data.py 509）**（app.js（前輪 43b7def 時 453）／styles.css（前輪 606）／tools/build-data.py）。鐵則 6 要求「既有檔 >500 被實質修改時觸發拆分 review 閘門：一句話答不出該檔職責 → 該次任務內提出拆分方案」，而三者在本區間各被改 **6／15／3** 次，BACKLOG 與 CHANGELOG 都找不到該閘門被行使的紀錄。**拆分候選（待拍板，不當場動工）**：① `app.js`（職責已含「資料載入 和 公式 和 食藥加成 和 分頁 和 init 接線」）→ 抽 `app-data.js`（loadData／11 支 JSON 的必要選配分類／索引建立）＋ `app-formula.js`（computeSettings／recipeMaxes／effectiveStats／applyConsumables／statShortfall），主檔只留 router／init／狀態持有；② `styles.css` → 按既有頁面邊界切成 `10-base`／`20-browse`／`30-recipe`／`40-result`／`50-quests` 序載（對齊 marketboard 的 `styles/NN-*.css` 慣例，同生態一致）；③ `tools/build-data.py` → 抽 `build_lib/`（每個資料源一個 module），主檔只留編排與 `PROBLEMS` 收尾。⚠️ 拆分產物一律直接適用 500 行上限；拆前先把「職責 → 檔名」對照表拍板，禁機械切行命名。 來源: 健檢 2026-09-05（R5） ✓ 完成於 cycle 2026-09-05-R5（三檔全拆：app.js→app-formula／app-data〔4fb67e7〕、build-data.py→build_lib/〔02ccfa7〕、styles.css→styles/10-base…50-tabs〔f2b4ceb〕；全部 <500 行、測試數不降、瀏覽器 smoke 過）

- [x] **B-035** (P2, perf)【建議 中｜延遲風險 中｜執行風險 中（跨 13 站）｜副作用 選 A 要改 13 站路由；選 B 只改文件與哨兵門檻｜體量 M】**2026-09-02 的 301 改動把首頁綁進 Pages Functions 日額度，且首頁無降級路徑**（_routes.json 的 include（"/", "/index.html"）＋ functions/_middleware.js:73；文件面 index.html:17-20）。為 SEO 把 `/` 與 `/index.html` 列進 include，代價是**每次開頁一次帳號級計費 invocation**；免費硬牆 100k/日是 **13 站共用**，而 2026-08-28 才剛因此全站 Functions 停擺過。本 repo 這側**首頁沒有任何降級路徑**（settings-api 有 503/502，首頁沒有）；`index.html:17-20` 的成本論證註解仍寫「新網域上的訪客一次 Function 都不用付」，已與事實矛盾。**需拍板**：A（建議）先驗 CF Pages 原生 `_redirects` 是否支援 host 級來源；若可，把舊 host 的 301 移出 Functions、`/` 退回純靜態；B 維持 Functions 但補三件事（改寫 index.html 成本註解／`check-cf-usage.py` 門檻把首頁流量計入／AGENTS 記一筆「首頁已在額度後面、無降級路徑」）；C 撤回 301 **不可行**（GSC 實查已證明 Google 否決 canonical、把 pages.dev 選成標準網址）。⚠️ **13 站共用同一形狀，不該單邊改**——本 repo 只做 B 的改註解那一項屬文件修正。 來源: 健檢 2026-09-05（R5） ✓ 完成於 cycle 2026-09-05-R5（Owner 拍板 A：301 改走 CF 帳號層 Bulk Redirects，middleware／inline 交接腳本／handoff 測試移除，首頁退出 Functions 計費面；commit 2dd6ca0。⚠ Bulk Redirects 是否已在帳號層生效只能在部署後由邊緣回應判定——本輪 curl 舊 host 回的 301 標頭與舊 middleware 完全相同，代表打到的仍是未部署前的舊版）

- [x] **B-036** (P2, correctness)【建議 中｜延遲風險 低｜執行風險 低｜副作用 職業切換列的鈕面文字改變｜體量 S】**一物多配方：同職多張時職業切換列出現多顆同名不可分辨的鈕**（app-recipe.js:222-235（jobSwitch）、:96-98（pickRecipeForItem））。模型假設「每職一張配方」，但主迴圈實算 `data/recipes.json`：**136 組（成品,職業）有 2〜4 張**（例：統一規格的金屬板 3 職 12 張，difficulty_factor 35〜234 差近 6 倍）。職業切換列以職業名為鈕面 ⇒ 同職多張時出現多顆同字串的鈕，玩家分不出點的是哪張；`pickRecipeForItem` 同職多筆時取先出現者、規則未明寫。三處註解「12 個＝全 DoH」與資料不符。**需拍板（UI 語彙）**：A（建議）鈕面帶可分辨資訊（rlv／難度），`pickRecipeForItem` 明寫「同職取難度最低」，golden：同職兩張 rlv 不同 ⇒ 鈕文字互異；B 先按職業分組、組內二次選擇；C 只修挑選規則、鈕不動。 來源: 健檢 2026-09-05（R5） ✓ 完成於 cycle 2026-09-05-R5（Owner 拍板 A：同職取難度最低、數值與原料全同的重複列去重、鈕面帶第一個有差異的數字或編號＋data-help 列原料；T52 +7；commit 71f3d0c）

- [x] **B-037** (P2, test)【建議 高｜延遲風險 中｜執行風險 中（改 13 repo 共用 hook）｜副作用 pre-commit 觸發更頻繁；canonicalTest 多一步｜體量 M】**機械網的觸發與安裝面四件同根：gate 6 抓不到 test-formulas.mjs／gate 3 看不到 .mjs／wasm 改動不觸發 cargo test／hooksPath 不進 git**（monorepo tools/check-test-baseline.js:198（--staged 觸發正則）；tools/git-hooks/pre-commit gate 3；.git/config core.hooksPath；fleet.json canonicalTest）。① `check-test-baseline.js --staged` 只在 staged 觸及 `tests?/`、`*.test.*`、`AGENTS.md` 時跑，而本 repo 654 條測試住在 `tools/test-formulas.mjs`——三條都不匹配 ⇒ 「悄悄刪幾條 T 案例」的 commit 剛好不會觸發 ratchet；② gate 3 副檔名清單只有 `.js`，`test-formulas.mjs` 3129 行遠超 2000 紅線、本區間被改 13 次零警告；③ `wasm/`（含 Cargo.lock）改動不觸發已宣告的 cargo test，「上游修好該撤 workaround」的絆線只能靠人想起；④ `core.hooksPath` 在 `.git/config` 指 `C:/FFXIVProject/tools/git-hooks`（repo 外絕對路徑），新 clone 靜默零閘門、repo 內無任何東西會發現；另 `deploy-prepare.sh` 不在 canonicalTest 也不在 pre-commit，第一次執行永遠在 CF 上。**需拍板（跨 repo hook＋fleet.json）**：A（建議）`--staged` 改由各 repo `TEST-BASELINE` 標記的 cmd／cwd 反推監看路徑、gate 3 補 `.mjs`、本 repo 加 `tests/hooks-installed.test.mjs`、canonicalTest 尾接 `sh deploy-prepare.sh`；B 只做本 repo 側（hooks 測試＋repo-local 行數斷言）；C 維持現狀。 來源: 健檢 2026-09-05（R5） ✓ 完成於 cycle 2026-09-05-R5（Owner 拍板 A：monorepo gate 6 --staged 由 TEST-BASELINE 反推監看路徑、gate 3 補 .mjs/.cjs〔monorepo 6414074〕；本 repo 加 tests/hooks-installed.test.mjs 與 tests/deploy-prepare.test.mjs〔7dbfaed〕。deploy-prepare 不接 canonicalTest 尾巴——fleet-check 對 canonicalTest 有 deploy 防呆，包成測試檔）

- [x] **B-038** (P3, build)【建議 中｜延遲風險 低｜執行風險 中（需 wasm-pack 工具鏈實跑）｜副作用 升級 nightly 變成有意識的 commit｜體量 S】**引擎重現性只釘了依賴沒釘工具鏈：rust-toolchain 裸 nightly、wasm-pack 版本無處可查**（wasm/rust-toolchain（channel = "nightly"）；wasm/BUILD-STAMP.json 欄位）。raphael-rs 釘 v0.26.2，但 `rust-toolchain` 是裸 `nightly`、`wasm-pack` 版本不在任何檔案裡 ⇒ 某天早上 nightly 變了就是一次建置事故，且無法回到「當初可編過的那一天」。**需拍板（要一次真實重建驗證才能收）**：A（建議）channel 釘 `nightly-YYYY-MM-DD`（現行可編那天），`build-wasm.ps1` 把 `rustc -V`／`wasm-pack -V` 寫進 BUILD-STAMP，`check-actions.py` 斷言 channel 形狀（裸 nightly 即紅）；B 只記版本不釘。 來源: 健檢 2026-09-05（R5） ✓ 完成於 cycle 2026-09-05-R5（Owner 拍板 A：channel 釘 nightly-2026-06-22、BUILD-STAMP 記 rustc／wasm-pack、check-actions 對帳 channel；真實重建 pkg hash 不變；commit bb2a215）

- [x] **B-039** (P3, test)【建議 中｜延遲風險 低｜執行風險 中（3270 行測試檔要按主題拆、harness 共用要抽層）｜副作用 canonicalTest 的 test-formulas 命令改成跑目錄；TEST-BASELINE 標記跟著改｜體量 M】**`tools/test-formulas.mjs` 3270 行過 2000 紅線，B-037 ② 補上 `.mjs` 後 gate 3 每次接觸都會印紅線警告**——這正是那條修法要的效果，但警告只在「下次實質接觸必須拆分」被履行時才停。候選：按既有 T 編號的主題分成 `tools/tests/` 下的數支（公式 golden／配方表與篩選／配方詳情與製作鏈／求解與巨集／職業任務／靜態哨兵），共用的 vm sandbox 工廠（`LAYER_STUBS`／`makeEl`／`mkT25Ctx`）抽成 `tools/tests/_harness.mjs`；入口改成掃描目錄跑全部並彙總 passed 數（match 正則不變）。⚠️ 拆分不得改任何斷言；每檔 ≤500 行。 來源: 2026-09-06 B-037 落地後 gate 3 首次對本檔發出紅線 ✓ 完成於 cycle 2026-09-05-R5（Owner 2026-09-06 拍板：入口檔名不變、tools/tests/ 13 支主題檔＋_harness，最大 351 行；684 條斷言集合 diff 為空、零順序依賴）
