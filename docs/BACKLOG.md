...
| B-007 | app.js 抽 `app-browse.js`（配方瀏覽層拆分） | 對抗審 2026-07-19（codex 阻擋 / grok 中） | **已完成**（2026-07-19，Owner 核可「有多個可拆分獨立功能可拆」）| app.js **502→437 行**（<500 達標）。**實作**：抽 `app-browse.js`（`globalThis.CraftBrowse`，91 行）＝ `renderChips`/`renderTable`/`markListState` ＋私有 `jobFilter`（僅本層讀寫）；`selectRecipe`/`showPicker`/`refreshSelectedGear`/公式/state **留 app.js**（選擇/詳情狀態機，耦合過重不宜同批移）。沿用 classic-script + deps 注入 pattern（同 app-render/app-solve；getter 取 live RINDEX/selected，注入 selectRecipe/toast）；app.js 以**同名 proxy** 委派 → 既有呼叫點/事件綁定/CraftList onChange 零改。瀏覽器實測整條瀏覽流程（表渲染 120/職業篩選/選配方收合 picker/返回列表/已加入綠底標示）零 console error；node --check/test-formulas 50 綠。 |
