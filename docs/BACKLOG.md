...
| B-007 | app.js 抽 `app-browse.js`（配方瀏覽層拆分） | 對抗審 2026-07-19（codex 阻擋 / grok 中） | **待拍板** | app.js **502 行 >500**（B-002 拆到 488 後、本輪配方表整合標示回長）；codex/grok 雙審點名 god-file 續脹、觸「>500 被實質修改 → 拆分 review」閘門。候選＝抽配方瀏覽層 `app-browse.js`（`renderChips`/`renderTable`/`markListState`/`selectRecipe`/`showPicker`）。⚠ **耦合重**：引用 `jobFilter`/`RINDEX`/`selected`/`ITEMS`/`JOB_ICON`/`iconUrl`/`esc`/`NAME_COLLATOR`/`CraftList`/`switchTab`/`invalidateResults`/`refreshSelectedGear`/`renderIngredients` → 需沿用 classic-script + deps 注入 pattern（同 app-render/app-solve），非機械切行。**本輪不當場做**：剛對抗驗證的 feature 上機械拆有回歸風險，且鐵則明訂「列候選給 Owner 拍板、不當場擴大 scope」。待 Owner `[go]` 授權。 |
