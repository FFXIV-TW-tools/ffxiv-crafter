...
## 交接頁測試為什麼不得改寫介面（B-048 Task 4；2026-09-05 隨交接機制退役，留作歷史）

> 2026-09-05：舊 host 的 301 改由 Cloudflare 帳號層 Bulk Redirects 執行，`functions/_middleware.js`／`tests/handoff.test.mjs`／`tests/route-manifest.json` 已刪。下文描述的是退役前的樣板約束。
...
> 以下各段於 2026-09-06 自 `AGENTS.md`「開發注意」搬入（健檢 R5 B-033：常駐層壓回豁免當時的 31,248 B 以下）。規則本體仍在 AGENTS.md，這裡是原文（含「怎麼發現的、錯了會怎樣」）。

## 首次提示的 CLS：為什麼是 parser-blocking 外部檔（2026-08-23）
...
## 最低能力要求：為什麼要擋、為什麼比 effectiveStats（2026-08-19）
...
## 版本篩選：為什麼由資料生成、為什麼 parseFloat（2026-08-19）
...
## 「繼續做」為什麼不推返回堆疊（Owner 2026-08-17）
...
## 遮罩關閉為什麼要看 mousedown（2026-08-17）
...
## 內容井為什麼要抽成一份（Owner 2026-08-19「看不清主次」）
...
## 配方表高度為什麼不能用 scrollHeight 反推（Owner 2026-08-19）
